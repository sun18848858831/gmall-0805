...
import com.atguigu.gmall.search.feign.GmallWmsCliet;
...
public class PmsListener {
    ...
    @Autowired
    private GmallWmsCliet wmsCliet;
    ...
    public void listener(Long spuId){
        ...
        if(!CollectionUtils.isEmpty(skuInfoEntities)){
            List<Goods> goodsList = skuInfoEntities.stream().map(skuInfoEntity -> {
                ...
                //查询库存信息
                Resp<List<WareSkuEntity>> wareSkuResp = this.wmsCliet.queryWareSkuBySkuId(skuInfoEntity.getSkuId());
                ...
                List<SearchAttrValue> searchAttrValues = attrValueEntities.stream().map(attrValueEntitie -> {
